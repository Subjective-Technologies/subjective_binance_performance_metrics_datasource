...
class SubjectivePerformanceMetricsDataSource(SubjectiveDataSource):
    connection_type = "PerformanceMetrics"
    connection_fields = ["portfolio_id", "start_date", "end_date", "run"]
    ...
    def fetch(self):
        ...
        from com_goldenthinker_trade_performance.PerformanceCalculator import PerformanceCalculator

        run = bool(self._get_param("run", False))
        if not run:
            self._emit_result({"status": "ready", "message": "Set run=True to execute performance calculation."})
        else:
            calculator = PerformanceCalculator()
            result = calculator.calculate_signals_perfornance()
